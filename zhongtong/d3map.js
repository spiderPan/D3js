...
// parse data properly
function drawDensityMap(data, map) {
    var umap = [];
    data.map(function(d) {
        umap[d[0]] = Number(d[1])
    });
    ...
    // LOAD DATA
    queue()
        .defer(d3.json, "data/maps/zh-mainland-provinces.topo.json") // mainland
        .defer(d3.json, "data/maps/zh-chn-twn.topo.json") // taiwan
        .defer(d3.json, "data/maps/zh-hkg-mac.topo.json") // hk and macau
    ...
    svg.append('g')
        .attr("class", "info")
        .attr("transform", "translate(" + (map.width - 140) + "," + (map.height - 180) + ")")
        .append("rect")
        .attr({
            fill: "transparent",
            height: 160,
    ...
    svg.select(".caption")
        .append("g")
        .attr("transform", "translate(" + (map.width - 25) + "," + (map.height - 25 - 5 * 49) + ")")
        .call(d3.svg.axis()
    ...
    svg.select('.caption')
        .append("g")
        .attr("class", "units")
        .attr("transform", "translate(" + (map.width - 35) + "," + (map.height / 2 - 20) + ")")
        .append("text")
        .attr("dx", function(d) {
            return 0
        })
    ...
    function drawTaiwan(error, cn) {
        ...
        svg.select(".map")
            ...
            .attr("class", "taiwan")
            .selectAll("path")
            .data(topojson.feature(cn, cn.objects.layer1).features.filter(function(d) {
    ...
    function drawHkMacau(error, cn) {
        ...
        svg.select('.map')
            .append("g")
            .attr("class", "hk")
            .attr("transform", "translate(50," + (map.height - 120) + ")")
            .selectAll("path")
